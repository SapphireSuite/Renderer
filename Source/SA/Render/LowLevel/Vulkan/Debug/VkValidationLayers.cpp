...
namespace SA::RND::VK
{
		...
		void* pUserData)
	{
		...
		switch (messageSeverity)
		{
			...
			{
				SA_LOG(pCallbackData->pMessage, Info, SA.Render.Vulkan.VLayers, (L"Vulkan Validation Layers %1", msgTypeStr));
				break;
			}
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
			{
				SA_LOG(pCallbackData->pMessage, Warning, SA.Render.Vulkan.VLayers, (L"Vulkan Validation Layers %1", msgTypeStr));
				break;
			}
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
			{
				SA_LOG(pCallbackData->pMessage, Error, SA.Render.Vulkan.VLayers, (L"Vulkan Validation Layers %1", msgTypeStr));
				break;
			}
			default:
			{
				SA_LOG(pCallbackData->pMessage, Normal, SA.Render.Vulkan.VLayers, (L"Vulkan Validation Layers %1", msgTypeStr));
				break;
			}
		}
